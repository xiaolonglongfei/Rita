/**
 * Run this via: pnpm --filter @workspace/scripts run migrate-to-supabase
 * It must be run inside the API server workflow context which has outbound network access.
 * Usage: called from api-server startup when SUPABASE_DATABASE_URL is set and tables are missing.
 */
import pg from "pg";
...
if (!SUPABASE_URL) {
  console.log("No SUPABASE_DATABASE_URL — nothing to migrate.");
  process.exit(0);
}
...
async function run() {
  ...
  try {
    ...
    const alexId  = users.find(u => u.email === "alex@example.com")!.id;
    const mariaId = users.find(u => u.email === "maria@example.com")!.id;
    ...
    const byName = (n: string) => instructors.find(i => i.name === n)!.id;

    // ── Seed reviews ─────────────────────────────────────────────────────────
    await client.query(`
      INSERT INTO reviews (user_id, instructor_id, value, effectiveness, punctuality, overall_score, comment, status) VALUES
    ...
    `, [
      alexId, mariaId,
      byName('Martin Miedzowicz'), byName('Claudia Bartolome'),
      byName('Jose Nunez Navarro'), byName('Patrick Hammers'),
      byName('Cesar Andrade'), byName('Vladimir Margalitadze'),
    ]);
    ...
    `);
    console.log("Instructor stats recomputed.");
