          ...
          >
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2">
                <span className="font-semibold text-rita-charcoal text-sm">{i.full_name}</span>
                {i.is_claimed && <CheckCircle2 className="h-3.5 w-3.5 text-rita-blue" />}
              </div>
              <p className="text-xs text-rita-gray">{i.teaching_locations ?? "No location"}</p>
            </div>
            <div className="text-right flex-shrink-0">
              <div className="text-sm font-bold text-rita-blue">
                {i.total_reviews > 0 ? i.avg_overall.toFixed(1) : "—"}
              </div>
